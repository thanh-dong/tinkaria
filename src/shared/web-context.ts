...
export function getWebContextPrompt(
  ...
  const promptLines = [
    ...
    "Supported embed formats are: `mermaid` for rendered diagrams, `svg` for rendered graphics, `html` for rendered embeds, `pug`/`pugjs` for compiled template embeds, `iframe` for remote embeds, `diashort` for remote embeds, and `d2` for source-only previews.",
    "HTML and Pug embeds run inside a sandboxed document shell with Tailwind v4 browser runtime available by default for quick mockups and UI sketches.",
