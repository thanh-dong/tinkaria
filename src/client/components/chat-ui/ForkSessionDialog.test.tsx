...
import { DEFAULT_FORK_PRESET_ID, getForkPreset } from "../../../shared/fork-presets"
...
describe("ForkSessionDialog", () => {
  ...
  test("exposes semantic ui identities while open", () => {
    expect(getForkSessionUiIdentities()).toEqual({
      ...
      presetAction: "chat.fork-session.preset.action",
      presetPopover: "chat.fork-session.preset.popover",
  ...
  test("backs fork-session grab targets with C3-owned descriptors", () => {
    ...
    expect(getUiIdentityAttributeProps(descriptors.presetAction)).toEqual({
      "data-ui-id": "chat.fork-session.preset.action",
      "data-ui-c3": "c3-110",
      "data-ui-c3-label": "chat",
    })
  })
  ...
  test("uses a non-empty default preset scaffold", () => {
    const preset = getForkPreset(DEFAULT_FORK_PRESET_ID)
